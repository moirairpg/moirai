...
public class AdaModel implements GptModel {

    private final GptService gptService;

    private static final String MODEL_ADA = "text-ada-001";
    private static final Logger LOGGER = LoggerFactory.getLogger(AdaModel.class);

    @Override
    public Mono<String> generate(final String prompt, final Persona persona) {

        LOGGER.debug("Called inference with Ada");
        return gptService.callCompletion(prompt, MODEL_ADA, persona).map(response -> {
