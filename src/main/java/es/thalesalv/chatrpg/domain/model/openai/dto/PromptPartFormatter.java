...
public class PromptPartFormatter implements Function<PromptPart, String> {
    ...
    public String apply(PromptPart part) {
        ...
        String suffix = Optional.ofNullable(part.getSuffix()).orElse(StringUtils.EMPTY);
        String toFormat =  prefix + content + suffix;
        Function<String,String> roleFormatter = Optional.ofNullable(roleFormatters.get(part.getRole())).orElse(Function.identity());
        Function<String,String> sourceFormatter = Optional.ofNullable(sourceFormatters.get(part.getSource())).orElse(Function.identity());
        return roleFormatter.andThen(sourceFormatter).apply(toFormat);
    }
}
